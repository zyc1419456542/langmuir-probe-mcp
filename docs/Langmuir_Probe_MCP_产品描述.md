...
**特色**: 自动检测阴极单放电 vs 阳极推力器的"同参数不同质"陷阱— Teff 相同但 EEPF 形状 KS=1.00。
...
| 模式判别 | SPOT / OSCILLATING / PLUME (基于 66 组 Kr 工质 B 场扫描标定) |
...
### 已验证: 霍尔推力器等离子体诊断
- 66 组 Kr 工质 B 场扫描 (SPOT→OSC→PLUME 模式转变)
- 36 组阴极单放电 (keeper 1A/0.5A, 0.2-3.0 sccm)
- 推力器羽流耦合区探针 (阴极正下方 1-2 cm)
- 电子密度范围: 10⁸ ~ 10¹⁰ cm⁻³
- 电子温度范围: 5 ~ 30 eV

### 可扩展: 
- 液滴/电喷雾推力器 (弱电离, 低密度 — 需调整平滑参数和电压范围)
- RF/ICP 等离子体源
- DC 辉光放电
- 任何使用圆柱单探针的等离子体诊断场景
...
4. Lobbia R B, Beal B E. *AIAA JPC* (2017). DOI:10.2514/1.B35531 — 电推进探针推荐实践
...
> **作者**: 许墨 | **实验数据**: 张翼辰 (长光卫星) & 方铁根 (清华大学能动系)
